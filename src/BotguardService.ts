...
export class BotguardService {
  ...
  public initializationPromise?: Promise<BG.BotGuardClient | undefined> | null =
    null;
  public integrityTokenBasedMinter?: BG.WebPoMinter;
  public bgChallenge?: DescrambledChallenge & {
    challenge?: string;
  ...
  private async _initBotguard() {
    ...
    if (this.bgChallenge) {
      ...
      const botguardResponse = await this.botguardClient.snapshot({
        webPoSignalOutput,
      });
      ...
      const integrityTokenResponseData =
        await integrityTokenResponse.json();
      const integrityToken = integrityTokenResponseData[0] as
        | string
        | undefined;
